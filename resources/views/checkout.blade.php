  ...
  fjs.parentNode.insertBefore(js, fjs);
	...
	<div class='afterLogo'>
		...
		<form method="POST" action="{{ route('order.store') }}">
			@csrf @if (session('selectedPlan') !== null)
			<h3>Pasirinktas planas: <span style='color: rgb(63,	115, 213);'><?php echo session('selectedPlan'); ?></span> <a href="{{ route('plans') }}"
				 class='change'>Keisti</a></h3>
			@else
			<h3>Planas nepasirinktas <span style='color: rgb(63,	115, 213);'><?php echo session('selectedPlan'); ?></span> <a href="{{ route('plans') }}"
				 class='change'>Pasirinkti</a></h3>
			@endif
    ...
    <select name='pardavimo_tipas' id='pardavimo_tipas'>
      ...
      <option value='nuoma'>Nuomuoti</option>
			...
			<div class="field">
					...
					<div class="control">
						<input class="input" type="text" placeholder="El. Paštas" name='email' required>
					</div>
				</div>
			<div class="field">
