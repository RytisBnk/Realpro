...
class phpinvoice extends FPDF_rotation  {
     ...
     ******************************************/
    public function __construct($size='A4',$currency='$',$language='lt') {
        $this->columns  		  	= 4;
        $this->items 			  = array();
        $this->totals 			 = array();
        $this->addText 			= array();
    ...
    public function addParagraph($paragraph) {
        ...
        $paragraph = iconv('UTF-8', 'cp1257', $paragraph);
    ...
    public function Body() {
        ...
        if($this->items) {
            foreach($this->items as $item)
            {
                ...
                $x = $this->GetX();
                $this->Cell($this->firstColumnWidth,$cHeight,iconv("UTF-8", "cp1257",$item['item']),0,0,'L',1);
                if($item['description']) {
                    $resetX = $this->GetX();
                    $resetY = $this->GetY();
                    ...
                    $this->MultiCell($this->firstColumnWidth,3,iconv("UTF-8", "cp1257",$item['description']),0,'L',1);
                    //Calculate Height
                    $newY = $this->GetY();
                ...
                if(isset($this->vatField) and $this->hide_vat == 0)
                {
                    $this->Cell($this->columnSpacing,$cHeight,'',0,0,'L',0);
                    if(isset($item['vat'])) {
                        $this->Cell($width_other,$cHeight,iconv('UTF-8', 'cp1257', $item['vat']),0,0,'C',1);
                    }
                    else
                    {
                ...
                if($this->curreny_direction == "left") {
                    $this->Cell($width_other,$cHeight,iconv('UTF-8', 'cp1257', $this->currency.' '.number_format($item['price'],2,$this->referenceformat[0],$this->referenceformat[1])),0,0,'C',1);
                }else{
                    $this->Cell($width_other,$cHeight,iconv('UTF-8', 'cp1257', number_format($item['price'],2,$this->referenceformat[0],$this->referenceformat[1]).' '.$this->currency),0,0,'C',1);
                }
                ...
                if(isset($this->discountField) and $this->hide_discount == 0)
                {
                    ...
                    {
                        $this->Cell($width_other,$cHeight,iconv('UTF-8', 'cp1257',$item['discount']),0,0,'C',1);
                    }
                    else
                    {
                        $this->Cell($width_other,$cHeight,'',0,0,'C',1);
                ...
                if($this->curreny_direction == "left") {
                    $this->Cell($width_other,$cHeight,iconv('UTF-8', 'cp1257', $this->currency.' '.number_format($item['total'],2,$this->referenceformat[0],$this->referenceformat[1])),0,0,'C',1);
                }else{
                    $this->Cell($width_other,$cHeight,iconv('UTF-8', 'cp1257', number_format($item['total'],2,$this->referenceformat[0],$this->referenceformat[1]).' '.$this->currency),0,0,'C',1);
                }
        ...
        if($this->totals)
        {
            foreach($this->totals as $total)
            {
                ...
                $this->Cell($width_other-1,$cellHeight,iconv('UTF-8', 'cp1257',$total['name']),0,0,'L',1);
                $this->Cell($this->columnSpacing,$cellHeight,'',0,0,'L',0);
                $this->SetFont($this->font,'b',8);
                $this->SetFillColor($bgcolor,$bgcolor,$bgcolor);
                if(is_bool($total['colored']) and $total['colored'] == true){
                ...
                $this->Cell($width_other,$cellHeight,iconv('UTF-8', 'cp1257',$total['value']),0,0,'C',1);
                $this->Ln();
                $this->Ln($this->columnSpacing);
            }
        }
        ...
        //Badge
        if($this->badge) {
            $badge = ' '.iconv('UTF-8', 'cp1257',$this->lithuanian_strtoupper($this->badge)).' ';
            $resetX = $this->getX();
            $resetY = $this->getY();
        ...
        foreach($this->addText as $text)  {
            if($text[0] == 'title')
            {
                ...
                $this->SetTextColor(50,50,50);
                $this->Cell(0,10,iconv("UTF-8", "cp1257//IGNORE",$this->lithuanian_strtoupper($text[1])),0,0,'L',0);
                $this->Ln();
            ...
            if($text[0] == 'paragraph')
            {
                ...
                $this->SetFont($this->font,'',8);
                $this->MultiCell(0,4,iconv("UTF-8", "cp1257",$this->WriteHTML($text[1])),0,'L',0);
                $this->Ln(4);
            }
        }
    ...
    public function WriteHTML($html) {
        ...
        foreach($a as $i=>$e) {
            if($i%2==0) {
                ...
                // Tag
                if($e[0]=='/')
                    $this->CloseTag($this->lithuanian_strtoupper(substr($e,1)));
                else {
                    // Extract attributes
                    $a2 = explode(' ',$e);
                    $tag = $this->lithuanian_strtoupper(array_shift($a2));
                    $attr = array();
                    foreach($a2 as $v) {
                        if(preg_match('/([^=]*)=["\']?([^"\']*)/',$v,$a3))
                            $attr[$this->lithuanian_strtoupper($a3[1])] = $a3[2];
                    }
    ...
    public function Header() {
        ...
        $this->SetFont($this->font,'B',9);
        $positionX = $this->document['w']-$this->margins['l']-$this->margins['r']-max($this->lithuanian_strtoupper($this->GetStringWidth($this->lang['number'])),
                $this->lithuanian_strtoupper($this->GetStringWidth($this->lang['date'])),
                $this->lithuanian_strtoupper($this->GetStringWidth($this->lang['due'])))-35;

        //Number
        if(!empty($this->reference)) {
            $this->Cell($positionX,$lineheight);
            $this->SetTextColor($this->color[0],$this->color[1],$this->color[2]);
            $this->Cell(32,$lineheight,iconv("UTF-8", "cp1257",$this->lithuanian_strtoupper($this->lang['number']).':'),0,0,'L');
            $this->SetTextColor(50,50,50);
            $this->SetFont($this->font,'',9);
        ...
        $this->SetFont($this->font,'B',9);
        $this->SetTextColor($this->color[0],$this->color[1],$this->color[2]);
        $this->Cell(32,$lineheight,iconv("UTF-8", "cp1257",$this->lithuanian_strtoupper($this->lang['date'])).':',0,0,'L');
        $this->SetTextColor(50,50,50);
        $this->SetFont($this->font,'',9);
        ...
        if(!empty($this->time)){
            ...
            $this->SetFont($this->font,'B',9);
            $this->SetTextColor($this->color[0],$this->color[1],$this->color[2]);
            $this->Cell(32,$lineheight,iconv("UTF-8", "cp1257",$this->lithuanian_strtoupper($this->lang['time'])).':',0,0,'L');
            $this->SetTextColor(50,50,50);
            $this->SetFont($this->font,'',9);
        ...
        if(!empty($this->due)){
            ...
            $this->SetFont($this->font,'B',9);
            $this->SetTextColor($this->color[0],$this->color[1],$this->color[2]);
            $this->Cell(32,$lineheight,iconv("UTF-8", "cp1257",$this->lithuanian_strtoupper($this->lang['due'])).':',0,0,'L');
            $this->SetTextColor(50,50,50);
            $this->SetFont($this->font,'',9);
        ...
        if($this->PageNo()== 1) {
            ...
            if($this->display_tofrom === true) {
                ...
                $this->SetTextColor(50,50,50);
                $this->SetFont($this->font,'B',10);
                $this->Cell($width,$lineheight,iconv('UTF-8', 'cp1257', $this->from[0]),0,0,'L');
        ...
        if(!isset($this->productsEnded))  {
            ...
            $this->Cell($this->firstColumnWidth,10,iconv("UTF-8", "cp1257",$this->lithuanian_strtoupper($this->lang['product'])),0,0,'L',0);
            $this->Cell($this->columnSpacing,10,'',0,0,'L',0);
            $this->Cell($width_other,10,iconv("UTF-8", "cp1257",$this->lithuanian_strtoupper($this->lang['qty'])),0,0,'C',0);
            if(isset($this->vatField) and $this->hide_vat == 0) {
                $this->Cell($this->columnSpacing,10,'',0,0,'L',0);
                $this->Cell($width_other,10,iconv("UTF-8", "cp1257",$this->lithuanian_strtoupper($this->lang['vat'])),0,0,'C',0);
            }
            $this->Cell($this->columnSpacing,10,'',0,0,'L',0);
            $this->Cell($width_other,10,iconv("UTF-8", "cp1257",$this->lithuanian_strtoupper($this->lang['price'])),0,0,'C',0);
            if(isset($this->discountField) and $this->hide_discount == 0) {
                $this->Cell($this->columnSpacing,10,'',0,0,'L',0);
                $this->Cell($width_other,10,iconv("UTF-8", "cp1257",$this->lithuanian_strtoupper($this->lang['discount'])),0,0,'C',0);
            }
            $this->Cell($this->columnSpacing,10,'',0,0,'L',0);
            $this->Cell($width_other,10,iconv("UTF-8", "cp1257",$this->lithuanian_strtoupper($this->lang['total'])),0,0,'C',0);
            $this->Ln();
            $this->SetLineWidth(0.3);
            $this->SetDrawColor($this->color[0],$this->color[1],$this->color[2]);
            $this->Line($this->margins['l'], $this->GetY(),$this->document['w']-$this->margins['r'], $this->GetY());
    ...
    public function Footer() {
        ...
        $this->Cell(0,10,iconv("UTF-8", "cp1257",$this->WriteHTML($this->footernote)),0,0,'L');
        $this->Cell(0,10,$this->lang['page'].' '.$this->PageNo().' '.iconv('UTF-8', 'cp1257', $this->lang['page_of']).' {nb}',0,0,'R');
    }

    private function lithuanian_strtoupper($string){
        foreach ($this->lt_symbols_toupper as $lower => $upper){
